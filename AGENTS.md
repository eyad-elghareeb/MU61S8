...
├── flashcard-engine.js         ← Flashcard deck engine (shared)
├── written-engine.js           ← Written/essay assessment engine (shared)
├── osce-engine.js              ← OSCE virtual patient engine (shared)
...
### Four engines — one per page type
...
| `flashcard-engine.js` | Flashcard deck files (`*-flashcards.html`) | `document.write()` snippet in flashcard file |
| `written-engine.js` | Written assessment files (`*-written.html`) | `document.write()` snippet in written file |
| `osce-engine.js` | OSCE virtual patient files (`*-osce.html`) | `document.write()` snippet in osce file |
...
### 4c. Flashcard File (`flashcard-engine.js` consumer)

```html
<!DOCTYPE html>
<html lang="en" data-theme="dark">
<head>
<!-- head identical to quiz, but uses 'flashcard-theme' for FOUC prevention -->
<title>Flashcard Deck Title</title>
</head>
<body>
<script>
...
### 4d. Index / Hub File (`index-engine.js` consumer)
...
### 4e. OSCE Virtual Patient File (`osce-engine.js` consumer)

```html
<!DOCTYPE html>
<html lang="en" data-theme="dark">
<head>
<!-- head identical to quiz file -->
<title>OSCE Virtual Patient</title>
</head>
<body>
<script>
/* [OSCE_CONFIG_START] */
const OSCE_CONFIG = {
  "uid": "osce_test_cases",         // REQUIRED — stable unique ID
...
- **written**: Contains `WRITTEN_CONFIG` + `WRITTEN_QUESTIONS`
- **flashcard**: Contains `FLASHCARD_CONFIG` + `FLASHCARD_BANK`
- **osce**: Contains `OSCE_CONFIG` + `OSCE_CASES`
...
med/osce/cardiology-osce.html
  └── osce-engine.js    (injects all CSS, HTML, logic — virtual patient SPA)
```
