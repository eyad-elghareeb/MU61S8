/* ================================================================
   exam-engine-shared.js  —  Shared runtime for quiz and bank pages.
   Booted by quiz-engine.js / bank-engine.js.
   ================================================================ */
(function () {
  'use strict';

  var _cs  = document.currentScript;
  var _boot = window.__MU61_EXAM_BOOT || {};
  var kind = _boot.kind === 'quiz' ? 'quiz' : 'bank';
  var IS_BANK = kind === 'bank';
  var IS_QUIZ = !IS_BANK;
  var ENGINE_BASE = _boot.engineBase || (_cs ? _cs.src.replace(/[^\/]*$/, '') : (window.__QUIZ_ENGINE_BASE || ''));
  var CONFIG = IS_BANK ? (window.BANK_CONFIG || {}) : (window.QUIZ_CONFIG || {});
  var QUESTION_SOURCE = IS_BANK ? (window.QUESTION_BANK || []) : (window.QUESTIONS || []);
  delete window.__MU61_EXAM_BOOT;

  function getQuestionIndices() {
    return Array.from({ length: QUESTION_SOURCE.length }, function (_, index) { return index; });
  }

  function getWorkingQuestions() {
    return SESSION_QUESTIONS.length ? SESSION_QUESTIONS : QUESTION_SOURCE;
  }
    ...
    <div class="mode-section">
      <div class="mode-grid">
        <label class="mode-label">
          ...
          <div class="mode-option mode-selected">
            <div class="mo-title">📝 Exam Mode</div>
            <div class="mo-sub">Timer + blind answers</div>
          </div>
        </label>
        <label class="mode-label">
          <input type="radio" name="quiz-mode" value="learning">
          <div class="mode-option">
            <div class="mo-title">📚 Learning Mode</div>
            <div class="mo-sub">Instant feedback</div>
          </div>
        </label>
      </div>
    </div>
...
<div id="result-screen" class="screen">
  ...
  <div class="result-body">
    ...
    <div class="result-list" id="result-list"></div>
    <div class="result-actions">
      <button class="btn-restart" onclick="onNewSessionClick(event)">↺ Restart Quiz</button>
      <a href="#" class="btn-restart btn-secondary" onclick="navigateToIndex(event); return false;">🏠 Return to Hub</a>
    </div>
  </div>
</div>
...
<div class="toast" id="toast"></div>
<!-- ════════════════════════════════════════════════════════════════
     ▼ QUIZ CONFIGURATION — Auto-injected by quiz-engine.js
═══════════════════════════════════════════════════════════════════ -->`;

  document.body.innerHTML = IS_QUIZ ? quizTemplate : bankTemplate;
